...
interface MetaGeneratorPageProps {
  tenantId: string;
  jwtToken: string;
}

export function MetaGeneratorPage({ tenantId, jwtToken }: MetaGeneratorPageProps) {
  const [activeTab, setActiveTab] = useState('generate');
  const [metaCount, setMetaCount] = useState(27);
      ...
      <div className="flex-1">
        <div className="max-w-[1400px] mx-auto px-6 py-6">
          ...
          <div className="text-[#6B7280] mb-4" style={{ fontFamily: 'Open Sans', fontSize: '14px' }}>
            Dashboard {'>'} Content Studio {'>'} Meta Generator
          </div>
          ...
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="text-[#6b7280]" style={{ fontFamily: 'Open Sans', fontSize: '16px' }}>
                Optimize titles and descriptions for AI platforms and search engines
              </p>
            </div>
            
            <button 
              className="text-[#6b7280] hover:text-[#2990C6] transition-colors"
              title="Uses discovery audit insights + ICP profile for AI-optimized meta tags"
            >
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="library" className="mt-0">
              <MetaLibrary onGenerateNew={() => setActiveTab('generate')} />
            </TabsContent>
          </Tabs>
        </div>
      </div>
