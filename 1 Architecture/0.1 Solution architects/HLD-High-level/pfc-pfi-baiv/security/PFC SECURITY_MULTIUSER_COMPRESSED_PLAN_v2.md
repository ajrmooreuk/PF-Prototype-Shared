# Security + Multi-User MVP: Compressed Implementation Plan v2.0
...
graph TB
    ...
    subgraph "Database Layer"
        CTX[set_tenant_context Function]
        RLS[Row Level Security Policies]
        TABLES[(Multi-Tenant Tables)]
        AUDIT[(Audit Log)]
    end
    
    UI -->|Request + Auth Token| MW
    MW -->|Extract Tenant/User| CTX
    CTX -->|Set Session Context| RLS
    MW --> API
    API -->|Query with RLS| TABLES
    TABLES -->|Audit Trigger| AUDIT
    RLS -->|Filter by Tenant| TABLES
```
...
sequenceDiagram
    ...
    participant ContextFn as set_tenant_context()
    participant Database
    participant RLS as RLS Policies
    participant AuditLog

    Client->>Middleware: API Request + JWT
    Middleware->>Middleware: Extract tenant_id, user_id, role
    Middleware->>ContextFn: set_tenant_context(tenant, user, role)
    ContextFn->>Database: SET app.current_tenant_id
    ContextFn->>Database: SET app.user_id
    ContextFn->>Database: SET app.user_role
    Middleware->>Database: Execute Query
    Database->>RLS: Check Policies
    RLS->>RLS: Verify tenant_id matches context
    RLS->>Database: Return filtered results
    Database->>AuditLog: Trigger audit_trigger_func()
...
```mermaid
erDiagram
    TENANTS ||--o{ ORGANIZATION_CYCLE_STATE : has
...
stateDiagram-v2
    ...
    Execution --> Audit: advance_cycle_stage()
...
sequenceDiagram
    ...
    participant API
    participant Presence as user_presence
    participant Locks as dataset_edit_locks

    U1->>API: POST /presence/heartbeat
    API->>Presence: UPSERT user_presence
    Presence-->>API: 200 OK
    
    U2->>API: GET /presence
    API->>Presence: SELECT active users
    Presence-->>API: User list
    API-->>U2: [User 1 online]
    
    U1->>API: POST /locks/acquire (dataset_123)
    API->>Locks: INSERT lock
    Locks-->>API: Lock acquired
    API-->>U1: 200 {lock_id}
    
    U2->>API: POST /locks/acquire (dataset_123)
    API->>Locks: Check existing locks
    Locks-->>API: Active lock exists
    API-->>U2: 409 Conflict (Locked by User 1)
    
    U1->>API: POST /locks/release (lock_id)
    API->>Locks: UPDATE is_active = false
    Locks-->>API: Released
    API-->>U1: 200 OK
```
...
graph LR
    ...
    ACTIVITY --> FEED[Activity Feed UI]
```
...
graph TB
    ...
    subgraph "Database Functions"
        ...
        ADV[advance_cycle_stage]
        TRIG[audit_trigger_func]
    end
    
    subgraph "Database Tables"
        TCYCLE[(organization_cycle_state)]
        TPRES[(user_presence)]
        TLOCK[(dataset_edit_locks)]
        TACT[(organization_activity)]
        TAUDIT[(audit_log)]
    end
...
**v2.0.0** - Added comprehensive mermaid diagrams:
- Architecture overview diagram
- Security flow sequence diagram
- Multi-user data model (ERD)
- Cycle stage state machine
- Presence & locking sequence diagram
- Activity logging flow diagram
- Complete system architecture diagram

**v1.0.0** - Initial compressed implementation plan
